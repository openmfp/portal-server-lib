...
import { ClientEnvironment, EnvVariables } from '../model/clientEnvironment';
...
export class EnvService {
  ...
  public getCurrentAuthEnv(hostname: string): ClientEnvironment {
    ...
    for (const baseDomainToIdp of baseDomainsToIdps) {
      ...
      const regExpExecArray = r.exec(hostname);
      if (!regExpExecArray) {
        continue;
      }
    ...
    throw new Error(
      `${hostname} is not listed in the portal's base urls: '${baseDomainsToIdps
        .map((x) => x.baseDomain)
        .join(',')}'`
    );
  }
  ...
  private getAuthEnv(idpName: string): ClientEnvironment {
